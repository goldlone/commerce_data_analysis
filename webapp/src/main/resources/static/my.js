  ...
  this.render = function () {
    ...
    this.showChart = function(platform, chartData) {
      var option = {
        ...
        legend: {
          data: ['新增会员', '新增用户', '活跃会员', '活跃用户', 'Session数量', '总会员', '总用户', '总Session时长'],
          selected: {
            "总会员": false,
            '总用户': false,
            '总Session时长': false
        ...
        xAxis: {
          type: 'category',
          boundaryGap: false,
          data: chartData[platform].date//['2018-12-05', '2018-12-06', '2018-12-07', '2018-12-08', '2018-12-09', '2018-12-10', '2018-12-11']
        },
        yAxis: {
          type: 'value'
        },
        series: [
          {
            ...
            type: 'line',
            stack: '总量',
            data: chartData[platform].new_members
          },
          {
            name: '新增用户',
            type: 'line',
            stack: '总量',
            data: chartData[platform].new_install_users
          },
          {
            name: '活跃会员',
            type: 'line',
            stack: '总量',
            data: chartData[platform].active_members
          },
          {
            name: '活跃用户',
            type: 'line',
            stack: '总量',
            data: chartData[platform].active_users
          },
          {
            name: 'Session数量',
            type: 'line',
            stack: '总量',
            data: chartData[platform].sessions
          },
          {
            name: '总会员',
            type: 'line',
            stack: '总量',
            data: chartData[platform].total_install_users
          },
          {
            name: '总用户',
            type: 'line',
            stack: '总量',
            data: chartData[platform].total_members
          },
          {
            name: '总Session时长',
            type: 'line',
            stack: '总量',
            data: chartData[platform].sessions_length
          }
        ]
      };
    ...
    // 获取用户基本信息数据
    $.ajax({
      url: rootPath + "/statsUser/selectAll",
      method: "post",
  ...
  this.render = function () {
    ...
    this.showChart = function(chartData) {
      var option = {
        ...
        title: { text: "活跃用户地域分布", /*subtext: "data from xxx\u7cfb\u7edf", sublink: "http://www.163.com",*/ left: "center", textStyle: { color: "#fff" } },
        tooltip: { trigger: "item" },
        legend: { orient: "vertical", y: "bottom", x: "right", data: ["xxx"], textStyle: { color: "#fff" } },
        geo: { map: "china", label: { emphasis: { show: !1 } }, roam: !0, itemStyle: { normal: { areaColor: "#323c48", borderColor: "#111" }, emphasis: { areaColor: "#2a333d" } } },
        series: [{
            name: "活跃用户分布", type: "scatter", coordinateSystem: "geo",
            data: convertData(chartData),
            tooltip: {
              enterable: true,
              trigger: 'item',
              formatter: function (params) {
                var marker = "<span style='display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background-color:#f4e925;'></span>";
                var res = "";
                res += "No." + (params.dataIndex + 1) + " " + params.name + "<br>";
                res += marker + "活跃用户：" + params.value[2] + "<br>";
                res += marker + "Session数量：" + params.value[3] + "<br>";
                res += marker + "跳出数：" + params.value[4];
                return res;
              }
            },
            symbolSize: function(a) { return a[2] / 3 },
    ...
    $.ajax({
      url: rootPath + "/location/selectAll",
      method: "post",
      success: function (res) {
        if(res.code === 1001) {
...
convertData = function (data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (geoCoord) {
      res.push({
        name: data[i].city,
        value: geoCoord.concat(data[i].activeUsers, data[i].sessions, data[i].bounceSessions)
      });
    }
  }
  return res;
